...
int iL = 0, iR = 0, pulseCount = 0, distance = 0, distanceR = 0, distanceL = 0;
int intDist = 0;
...
void loop() {
  ...
  //   int dist = ((joyY + 102) * 20) / 1023;

  //   while (newDistance != dist) {
  //     Serial.println(joyY);
  //     Serial.println(dist);
  //     newDistance = myLIDAR.getDistance();
  //     if (newDistance < dist) {
...
void ex3step1() {
 ...
 if (turnCount < 2) {
    if (!turn) {
      if (turnCount == 0 && pulseCount <= 300) {
        pwm_R = 255;
        pwm_L = 255;
        digitalWrite(Motor_R_dir_pin, Motor_forward);
        digitalWrite(Motor_L_dir_pin, Motor_forward);
      } else if (turnCount == 1 && pulseCount <= 500) {
        pwm_R = 255;
        pwm_L = 255;
        digitalWrite(Motor_R_dir_pin, Motor_forward);
        digitalWrite(Motor_L_dir_pin, Motor_forward);
...
void ex3step2(){
  ...
  if (intDist > 0) {
    if (turnCount2 < 2) {
      if (!turn2) {
        if (turnCount2 == 0 && pulseCount <= pulsesForStatcm) {
          pwm_R = 255;
          pwm_L = 255;
          digitalWrite(Motor_R_dir_pin, Motor_forward);
          digitalWrite(Motor_L_dir_pin, Motor_forward);
        } else if (turnCount2 == 1 && pulseCount <= pulsesForStatcm) {
          pwm_R = 255;
          pwm_L = 255;
          digitalWrite(Motor_R_dir_pin, Motor_forward);
          digitalWrite(Motor_L_dir_pin, Motor_forward);
...
void displayToLcd(byte raw) {
  ...
  //display distance
  distanceR = (iR * wheelDistanceRevolution) / PPR;
  distanceL = (iL * wheelDistanceRevolution) / PPR;
  // lcd.setCursor(0, 3);
  // lcd.print("D_R:");
  // lcd.print(distanceR);
  // lcd.print(" D_L:");
